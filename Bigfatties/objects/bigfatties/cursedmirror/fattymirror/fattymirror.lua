...
function init()
  ...
  self.chatTimer = 0
  self.sizes = root.assetJson("/scripts/bigfatties.config:sizes")
	self.settings = root.assetJson("/scripts/bigfatties.config:settings")
...
function use(args)
  if storage.active then
    ...
    promises:add(world.sendEntityMessage(targetId, "bigFatties.getData", entity.id()), function(data)
      currentSize, currentSizeIndex = getSize(data.weight)
      if currentSizeIndex == #self.sizes then return end
      local currentProgress = (data.weight - currentSize.weight)/(self.sizes[currentSizeIndex + 1].weight - currentSize.weight)
      local amount = math.floor(0.5 + self.sizes[currentSizeIndex + 1].weight - data.weight + currentProgress * ((self.sizes[currentSizeIndex + 2] and self.sizes[currentSizeIndex + 2].weight or self.settings.maxWeight) - self.sizes[currentSizeIndex + 1].weight))
      world.sendEntityMessage(targetId, "bigFatties.gainWeight", amount)
      deactivate()
