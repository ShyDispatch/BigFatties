...
function build(directory, config, parameters, level, seed)
  ...
  if not config.inventoryIcon then
    local tier = 1
    if config.builderConfig.tier then
      tier = math.min(config.builderConfig.tier or 1, 3)
    end
    config.inventoryIcon = string.format("%s.png:%s%s%s", configParameter("accessoryType", "ring"), tier, "?replace;00000000=00000001", config.paletteSwaps)
  end
  ...
  if not parameters.stats then
    ...
    for i=1, statCount do
      ...
      experienceMultiplier = experienceMultiplier + (modifier * -1 * stat.weight)
    end

    table.sort(parameters.stats, function(a, b) return a.name < b.name end)

    if config.statConfig.experienceMultiplier then
      parameters.stats[#parameters.stats + 1] = {name = "experienceMultiplier", modifier = experienceMultiplier}
    end
  end
