function build(directory, config, parameters, level, seed)
  ...
  if not configParameter("effectApplied", false) then
    ...
    table.insert(effects[1], {effect = "bigfattiesfood", duration = configParameter("foodValue", 0)})
    table.insert(effects[1], {effect = "bigfattiesexperience", duration = configParameter("foodValue", 0) * experienceRatio[string.lower(configParameter("rarity", "common"))]})
